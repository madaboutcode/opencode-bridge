//! The squarified-treemap algorithm (Bruls et al.), used unmodified —
//! `layout.md` R5. Ported verbatim from the verified spike
//! (`tmp/20260901-prototype-dashboard-layout/src/squarify.rs`); this file
//! has no domain knowledge at all (no session/project/attention-state
//! concept), so there was nothing to adapt for real data — see that spike
//! file's own tests, reproduced below, for the behavior this port is
//! required to match exactly (T11 contract, acceptance criterion 1).

use ratatui::layout::Rect;

/// A leaf item for the squarified treemap. `label` is unused by the
/// algorithm itself — kept only so a caller building `TreemapItem`s can
/// carry a human-readable tag through for debugging, same as the spike.
#[derive(Debug, Clone)]
...
// Index-based loops throughout this function are kept verbatim from the
// verified spike (`tmp/20260901-prototype-dashboard-layout/src/squarify.rs`)
// rather than rewritten to satisfy clippy's iterator-style preference —
// several of these indices (`idx`, `j`) are also used in arithmetic beyond
// plain indexing (`is_last_tile`, `remaining_tiles`), and this is the one
// module the T11 contract asks to be a byte-faithful port with "no
// behavioral change from the spike's verified output shape" (acceptance
// criterion 1); a mechanical rewrite risks exactly the kind of subtle
// off-by-one the port is supposed to avoid.
...
mod tests {
    ...
    fn item(id: &str, weight: f64) -> TreemapItem {
        TreemapItem {
            id: id.into(),
            weight,
        }
    ...
    fn equal_weights_similar_aspect() {
        let items: Vec<TreemapItem> = (0..4).map(|i| item(&format!("s{i}"), 1.0)).collect();
        ...
        for (_, rect) in &result {
            ...
            assert!(
                ratio < 4.0,
                "aspect ratio {ratio} too high for rect {rect:?}"
            );
        }
    }
    ...
    #[test]
    fn unequal_weights() {
        let items = vec![item("a", 3.0), item("b", 1.0)];
        let result = squarify(&items, Rect::new(0, 0, 80, 20));
    ...
    fn zero_weight_items_ignored() {
        let items = vec![item("a", 0.0), item("b", 1.0)];
        let result = squarify(&items, Rect::new(0, 0, 80, 20));
        assert_eq!(result.len(), 1);
    }
    ...
    #[test]
    fn no_sliver_bars() {
        let items: Vec<TreemapItem> = (0..3).map(|i| item(&format!("s{i}"), 1.0)).collect();
        let result = squarify(&items, Rect::new(0, 0, 80, 24));
