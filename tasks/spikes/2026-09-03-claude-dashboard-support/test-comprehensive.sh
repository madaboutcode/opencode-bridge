...
WORK_DIR=$(mktemp -d "$BASE_DIR/claude-comprehensive-XXXXXX")
...
# Create settings.json with hooks for all events we care about
# Parameter: SYNC or ASYNC
HOOK_TYPE=${1:-SYNC}
...
echo "=== Running claude --print with $HOOK_TYPE hooks from disposable cwd ==="
...
# Wait for hooks to complete (different for sync vs async)
if [ "$HOOK_TYPE" = "ASYNC" ]; then
    echo "Waiting 5 seconds for async hooks to complete..."
    sleep 5
else
    echo "Waiting 2 seconds for synchronous hooks to complete..."
    sleep 2
fi
