...
| implementer | M1-M2: `coder` agent. **M3: opencode `deepseek` agent** — dispatched via `mcp__opencode-bridge__opencode_task` with `agent: "deepseek"` (never the `model` param). Role binding changed mid-run per user instruction 2026-09-02 — see `decisions.md`. | M1-M2: per user instruction, coder for runner and impl roles. |
| reviewer | M1-M2: `ask_opus` agent. **M3: opencode `glm-5.3` agent** — same dispatch mechanism, `agent: "glm-5.3"` (never `model`). Role binding changed mid-run per user instruction 2026-09-02 — see `decisions.md`. | M1-M2: per user instruction, opus for review. **Calibration, not a checklist** (see refine-loop's own guardrail against "focus on X, Y, Z" phrasing) — achieved via each contract's Context line: scale/criticality set low for spike work (M1), which is what actually suppresses edge-case pedantry through refine-loop's built-in "in scope only if it threatens this task's stated goal at this task's stated scale" rule. Reviewer is always told which skill(s) to read and apply (`code-quality`, plus `software-design` for anything with real component boundaries) and is always given the contract's Acceptance criteria as the adherence-to-spec check. M3: runner (`coder`) dispatches both implementer and reviewer turns with `wait: true` on `opencode_task` — the refine-loop is synchronous by design, and the bridge's async default risks a callback landing after the runner has already reported back. **M3 role binding is a one-task trial first, per user instruction — see below before treating it as the standing binding for all of M3.** |
